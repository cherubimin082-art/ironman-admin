...
// ── Main page ───────────────────────────────────────────────────
export default function IronBoysPage() {
  const { isMobile } = useWindowSize();

  const [ironBoys, setIronBoys] = useState([]);
  ...
  const [form, setForm] = useState({ name: "", phone: "", vendor_id: "" });

  const load = useCallback(async () => {
    setLoading(true); setError("");
    try {
      const [boysRes, vendorsRes] = await Promise.all([
        api.get("/admin/iron-boys"),
        api.get("/vendors"),
      ]);
      setIronBoys(boysRes.data.iron_boys || []);
  ...
  function openEdit(b) {
    setSelected(b);
    setForm({ name: b.name || "", phone: b.phone || "", vendor_id: b.vendor_id ? String(b.vendor_id) : "" });
    setFormErr(""); setFormOk("");
    setModal("edit");
  }
  ...
  async function handleAdd(e) {
    ...
    if (!form.name.trim() || !form.phone.trim() || !form.vendor_id)
      return setFormErr("Name, mobile number and vendor are required");
    setSaving(true); setFormErr(""); setFormOk("");
    try {
      await api.post("/admin/iron-boys", form);
      setFormOk("Iron boy created successfully");
      await load();
  ...
  async function handleEdit(e) {
    ...
    setSaving(true); setFormErr(""); setFormOk("");
    try {
      await api.put(`/admin/iron-boys/${selected.id}`, form);
  ...
  async function handleDelete() {
    setSaving(true); setFormErr(""); setFormOk("");
    try {
      await api.delete(`/admin/iron-boys/${selected.id}`);
      setFormOk("Removed successfully");
      await load();
      setTimeout(closeModal, 900);
    } catch (err) {
      setFormErr(err.response?.data?.message || "Failed to remove");
    } finally { setSaving(false); }
  }
  ...
  return (
    <Layout>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13.5, color: "#9ca3af", margin: "6px 0 0", fontWeight: 400 }}>
              Create iron boy accounts — they log in with mobile number + OTP and get the same start/complete ironing screen the old shared tablet used, scoped to their assigned vendor shop.
            </p>
          </div>
          <button
            onClick={openAdd}
        ...
        <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e5e7eb", boxShadow: "0 2px 10px rgba(0,0,0,0.05)" }}>
          <div style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: "#374151" }}>
              {loading ? "Loading…" : `${ironBoys.length} Iron Boy${ironBoys.length !== 1 ? "s" : ""} Registered`}
            </span>
          ...
          {loading ? (
            ...
            <div style={{ padding: "60px 24px", textAlign: "center" }}>
              <p style={{ fontSize: 15, fontWeight: 700, color: "#374151", margin: "0 0 6px" }}>No iron boys yet</p>
              <p style={{ fontSize: 13, color: "#9ca3af", margin: 0 }}>Click "Add Iron Boy" to create the first one.</p>
            </div>
          ) : (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 560 }}>
                <thead>
                  <tr style={{ background: "#f9fafb", borderBottom: "1px solid #f3f4f6" }}>
                    {(isMobile ? ["Name", "Vendor", "Actions"] : ["#", "Name", "Mobile Number", "Vendor Shop", "Actions"]).map(h => (
                      <th key={h} style={{
                ...
                <tbody>
                  {ironBoys.map((b, idx) => (
                    <tr
                      key={b.id}
                      style={{ borderBottom: idx < ironBoys.length - 1 ? "1px solid #f9fafb" : "none" }}
      ...
      {/* ── Add Modal ── */}
      {modal === "add" && (
        <Modal title="Add New Iron Boy" onClose={closeModal}>
          <form onSubmit={handleAdd} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              <label style={labelSt}>Vendor Shop *</label>
            ...
            <div>
              ...
              <input style={inputSt} placeholder="e.g. Ramesh" value={form.name}
            ...
            <div>
              ...
              <p style={{ fontSize: 11, color: "#9ca3af", margin: "5px 0 0" }}>
                No password needed — they log in with this number + a WhatsApp OTP.
              </p>
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button type="submit" disabled={saving} style={{
                ...
                background: saving ? "#e5e7eb" : "linear-gradient(135deg,#DC2626,#B91C1C)",
                color: saving ? "#9ca3af" : "#fff",
                fontSize: 13, fontWeight: 700,
                cursor: saving ? "not-allowed" : "pointer",
              }}>{saving ? "Creating…" : "Create Iron Boy"}</button>
            </div>
          </form>
        </Modal>
      )}
      ...
      {modal === "delete" && selected && (
        <Modal title="Remove Iron Boy" onClose={closeModal} maxWidth={440}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{
              ...
              <p style={{ fontSize: 14, fontWeight: 700, color: "#dc2626", margin: "0 0 8px" }}>
                ⚠️ Deactivates this account
              </p>
              <p style={{ fontSize: 13, color: "#374151", margin: 0, lineHeight: 1.6 }}>
                Remove <strong>{selected.name}</strong>? Their login will stop working and they'll disappear from this list — past ironing activity is kept.
              </p>
            </div>
            <Alert type="err" msg={formErr} />
            <Alert type="ok"  msg={formOk}  />
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleDelete} disabled={saving} style={{
                ...
                fontSize: 13, fontWeight: 700,
                cursor: saving ? "not-allowed" : "pointer",
              }}>{saving ? "Removing…" : "Yes, Remove"}</button>
            </div>
          </div>
        </Modal>
      )}
